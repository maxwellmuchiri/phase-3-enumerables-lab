...
def print_spiciest_foods(spicy_foods)
  # your code here
  spiciest_foods = spicy_foods.select { |spicy_food| spicy_food[:heat_level] > 5 }
  spiciest_foods.each do |spicy_food|
    heat_level_emoji = "🌶" * spicy_food[:heat_level]
    puts "#{spicy_food[:name]} (#{spicy_food[:cuisine]}) | Heat Level: #{heat_level_emoji}"
  end
end
...
def average_heat_level(spicy_foods)
  ...
  average_heat_level = total_heat_level / spicy_foods.count
  average_heat_level.to_i
end
